...
def get_split_domain_info_from_dir(domain_path, dataset_name=None, val_percentage=None, domain_label=None):
    ...
    if dataset_name == "VLCS":
        ...
        --------------------------------------------------
        {'dog': 1, 'elephant': 2, 'giraffe': 3, 'guitar': 4, 'horse': 5, 'house': 6, 'person': 7}
        --------------------------------------------------
        '''
        for i, item in enumerate(classes):
            ...
            for root, _, fnames in sorted(os.walk(class_path)):
                for fname in sorted(fnames):
                    ...
                    '''
                    print(path)
                    art_painting/dog/pic_001.jpg
                    art_painting/dog/pic_002.jpg
                    art_painting/dog/pic_003.jpg
                         .........
                    '''
                    names.append(path)
...
def get_split_dataset_info_from_txt(txt_path, domain, domain_label, val_percentage=None):
    # txt_path=/data/wangna/dataset/PACS/pacs_label
    if "PACS" in txt_path:
        train_name = "_train_kfold.txt"
        val_name = "_crossval_kfold.txt"

        # train_txt = / data / wangna / dataset / PACS / pacs_label / photo_train_kfold.txt
        # val_txt = / data / wangna / dataset / PACS / pacs_label / photo_crossval_kfold.txt
        train_txt = txt_path + "/" + domain + train_name
        val_txt = txt_path + "/" + domain + val_name


        # 返回两个列表 art_painting/dog/pic_225.jpg   1
       ...
       # print(train_domain_labels)  [1,1,1,1...]  [2,2,2,...]  [3,3,3,...]   三个列表长度不同
...
from data.high_pass import HighPass
...
class JigsawDatasetRandAug(data.Dataset):
    def __init__(self, names, labels, domain_labels, patches=True, dataset_path=None, img_transformer=None,
        ...
        # self._image_transformer = img_transformer
        self._image_transformer = img_transformer
        self._image_transformer_aug = HighPass(args)  #RandAugment(args)
    ...
    def __getitem__(self, index):
        framename = self.data_path + '/' + self.names[index]
        img = Image.open(framename).convert('RGB')
        img_randaug = self._image_transformer_aug(img)
